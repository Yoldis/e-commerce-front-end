...
export const RegisterPage = () => {
  const { name, email, password, onChangeInput } = useForm<FormInput>({
    name: "",
    email: "",
    password: "",
  });
  ...
  const { registerDispatch, loading } = useUserStoreHook();

  const onSubmit = async (e: ChangeEvent<HTMLFormElement>) => {
    setErrors({ name: "", email: "", password: "" });
    e.preventDefault();
    const resp = registerSchema.safeParse({ name, email, password });
    if (!resp.success) {
    ...
    registerDispatch(resp.data.name, resp.data.email, resp.data.password);
    setErrors({ name: "", email: "", password: "" });
  };
      ...
      <div className="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
        <div className="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
          ...
          <div className="text-center text-sm font-semibold py-5">
            <h1 className="mb-3">Ya tienes cuenta?</h1>
            <Link
              to={"/auth/login"}
